...
class TestStoreAPI:
    ...
    def test_get_find_purchase_by_id(self, api_store, get_id_from_order_data):
        ...
        response = api_store.get_store_order(get_id_from_order_data)
        print("JSON-ответ от API:", f"Code: {response.status_code}", response.json())
        self.validator.validate_json_response(response)
        self.validator.validate_json_value(response, "id", get_id_from_order_data)
    ...
    def test_delete_purchase_by_id(self, api_store, get_id_from_order_data):
        ...
        response = api_store.delete_store_order(get_id_from_order_data)
        print("JSON-ответ от API:", f"Code: {response.status_code}", response.json())
        self.validator.validate_json_response(response)
        self.validator.validate_json_value(response, "message", str(get_id_from_order_data))